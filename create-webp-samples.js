// Создание примеров WebP карт для тестирования
const fs = require('fs');
const path = require('path');

console.log('🎨 Creating WebP sample cards...');

// Создаем несколько примеров WebP карт для каждой темы
const sampleCards = [
    // Casino theme samples
...
// Создаем HTML файл для генерации WebP карт
const htmlContent = `
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>WebP Cards Generator</title>
    <style>
...
<body>
    ...
    <script>
        ...
        function createCard(card) {
            ...
            cardDiv.innerHTML = \`<div><strong>\${card.rank}\${card.suit}</strong><br>\${card.theme}<br>\${card.filename}</div>\`;
        ...
        function downloadAllCards() {
            ...
            allCanvases.forEach((canvas, index) => {
                setTimeout(() => {
                    downloadCard(canvas, cards[index].filename);
                }, index * 200);
            });
        }
...
fs.writeFileSync('webp-cards-generator.html', htmlContent);

console.log('✅ Created webp-cards-generator.html');
console.log('📋 Instructions:');
console.log('1. Open webp-cards-generator.html in your browser');
console.log('2. Click "Download All Cards" to get all WebP files');
console.log('3. Place the downloaded files in the appropriate WEBP_cards folders');
console.log('4. Test the game with WebP support');

console.log('\n🎯 Expected results:');
console.log('• Casino: SVG → WebP (30-50% size reduction)');
console.log('• Tavern: PNG → WebP (50-70% size reduction)');
console.log('• Underground: JPG → WebP (30-50% size reduction)');
